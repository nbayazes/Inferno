...
float3 Atmosphere(float3 origin, float3 rd) {
    ...
    col += pow(horizon, 30.) * float3(0.4, 0.5, 1) * 2;
    col += pow(horizon, 2.) * float3(0.6, 0.7, 1) * 0.02;
    return col;
}

float4 psmain(PS_INPUT input) : SV_Target {
    float2 q = input.pos.xy / Frame.Size;
    float2 p = q - 0.5;
    ...
    float3 rd = normalize((p.x * -right + p.y * Frame.EyeUp) * radius - Frame.EyeDir);
    rd = normalize(rd);

    //float3 rd = normalize((p.x + p.y) * 1.05 + Frame.EyeDir);
    //float3 rd = Frame.EyeDir;

    float3 origin = float3(0., 15, 0); // position of scattering
    float3 rdScatter = rd;
    rdScatter.y -= abs(p.x * p.x * 0.035); // surface curvature
    float3 atmosphere = Atmosphere(origin, rdScatter);

    // Scale starfield to same pixel density regardless of resolution
    // This does mean higher resolution displays will have more stars, but they look odd when larger than a few pixels
    float scale = 1440 / Frame.Size.y; 
    float density = 0.25;
    float3 bg = Starfield(rd, scale, density) * (1 - saturate(dot(atmosphere, float3(1, 1, 1))));
    return float4(bg + atmosphere, 1);
